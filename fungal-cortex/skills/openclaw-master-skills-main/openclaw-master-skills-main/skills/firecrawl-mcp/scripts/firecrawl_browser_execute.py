...
def validate_required(payload):
    missing = [key for key in ['sessionId', 'code'] if key not in payload]
    if missing:
        raise SystemExit(f"Missing required fields: {', '.join(missing)}")


def main():
    parser = argparse.ArgumentParser(description="firecrawl-mcp firecrawl_browser_execute")
    ...
    result = router.invoke(
        ...
        api_id="firecrawl_browser_execute",
        data=payload,
    )
