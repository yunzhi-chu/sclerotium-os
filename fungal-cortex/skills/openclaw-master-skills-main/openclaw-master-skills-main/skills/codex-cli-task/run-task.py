    ...
    if channel == "whatsapp" and session_key:
        ...
        if iterative_intent:
            wake_text = (
                ...
                "This is a system wake call after OpenAI Codex has sent the result of your last run. "
                "Consider this a normal continuation of the same dialogue: the same agent persona, the same session, the same history.\n\n"
                "Key rule of continuity: focus on the ORIGINAL user goal for which OpenAI Codex was launched, "
                "not just the latest technical sub-step. If the current sub-step is complete but the original goal is not — continue moving towards the original goal.\n"
                "If you see that this is an outdated wake for an already closed newer iteration — briefly mark it as stale and do not run anything.\n\n"
                "Create a natural continuation of the correspondence. For debugging, use the prefix at the beginning of the message: "
                "[TRACE][AGENT][WAKE_RECEIVED].\n"
                "STRICT RULE: silent launch is forbidden. First send a separate visible turn with analysis and decision ([TRACE][AGENT][DECISION]). "
                "Only AFTER that, as a separate next action, launch the next iteration if necessary.\n"
                "1) briefly react to the OpenAI Codex result,\n"
                "2) compare the result with the original user goal and check if it is fully achieved,\n"
                "3) if the original goal is not achieved — explicitly report a 'continue' decision and what exactly you are refining, then launch exactly one next iteration,\n"
                "4) if the original goal is achieved — explicitly report a 'stop' decision and stop."
            )
        else:
            wake_text = (
                ...
                "Consider this a normal continuation of the same dialogue: the same agent persona, the same session, the same history.\n\n"
                "Key rule of continuity: focus on the ORIGINAL user goal for which OpenAI Codex was launched, "
                "not just the latest technical sub-step.\n"
                "If you see that this is an outdated wake for an already closed newer iteration — briefly mark it as stale and finish without new actions.\n\n"
                "Create a natural continuation of the correspondence. For debugging, use the prefix at the beginning of the message: "
                "[TRACE][AGENT][WAKE_RECEIVED].\n"
                "STRICT RULE: silent launch is forbidden. Give a separate visible turn with analysis and an explicit decision ([TRACE][AGENT][DECISION]).\n"
                "1) briefly react to the result,\n"
                "2) evaluate if the user's original goal has been fully achieved,\n"
                "3) report the outcome to the user and stop."
            )
