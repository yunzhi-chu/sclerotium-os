...
================================
Polymarket天气市场批量交易机器人

核心功能：
- 预生成29城市×2天的event slugs（今明两天）
- 异步获取Gamma Events + CLOB Orderbooks
- 本地时间09:00-09:55监控，每5分钟检查
- 触发条件：CLOB best_ask ≥ 0.35
- 买入1 share YES，按best_ask价（有15%滑点保护）
- 10:00 fallback：未持仓的event选最高CLOB中间价outcome买入
- 每4分钟打印positions/trades/PnL报告
- 磁盘缓存+限流+重试
...
用法：
  ...
  --once      只运行一次扫描后退出（调试用）
  --status    显示当前持仓和统计
  --live      真实交易（需要配置 API 凭证）

实盘配置（--live 模式）：
  1. 安装依赖: pip install py-clob-client
  2. 设置环境变量:
     export PRIVATE_KEY="0x..."           # EOA 钱包私钥
     export PROXY_WALLET="0x..."         # Polymarket 代理钱包地址（从网站导出）
     export SIGNER_TYPE="2"              # 0=EOA, 2=Gnosis Safe（推荐）
  3. 首次运行会自动派生 L2 API 凭证（API key/secret/passphrase）
  4. L2 凭证保存到内存，会话期间自动使用

认证机制：
  - L1: EIP-712 签名（私钥） → 派生 API 凭证
  - L2: HMAC-SHA256（API 凭证） → 下单/撤单/查询
  订单本身需要 EIP-712 签名，请求头需要 HMAC 签名
"""
...
try:
    ...
    print(f"[WARN] py-clob-client 不可用: {e}")
    print("[WARN] 请安装: pip install py-clob-client")
...
# =============================================================================
# SkillPay Billing Functions (按订单计费 - 每次成功下单扣 0.01 USDT)
# =============================================================================
import requests
...
def billing_charge(user_id: str) -> dict:
    """
    扣费：每次成功下单后调用
    使用 skill 预设费率（在 SkillPay 后台配置，当前为 0.01 USDT/次）
    ...
    try:
        ...
        print(f"[ERROR] 计费失败 (网络错误): {e}")
        return {'ok': False, 'balance': 0.0}
    except Exception as e:
        print(f"[ERROR] 计费失败: {e}")
        return {'ok': False, 'balance': 0.0}
...
def billing_get_payment_link(user_id: str, amount: float = 8) -> str:
    ...
    try:
        ...
        print(f"[ERROR] 获取充值链接失败 (网络错误): {e}")
        return ""
    except Exception as e:
        print(f"[ERROR] 获取充值链接失败: {e}")
        return ""
...
def load_state() -> dict:
    ...
    if STATE_FILE.exists():
        ...
        if positions:
            ...
            if not any(is_city_date_key(k) for k in positions.keys()):
                ...
                print("[MIGRATE] 状态格式已从 market_id keys 迁移到 city_date keys")
    ...
    return {
        ...
        "last_report": None,
        "last_full_scan": None,
        "fallback_executed": [],  # 已执行fallback的city_date列表
    }
...
async def init_auth_manager() -> PolymarketAuth:
    """
    初始化认证管理器。
    - .env 已有 POLY_API_KEY/SECRET/PASSPHRASE → 直接加载，继续运行
    ...
    if api_key and api_secret and api_passphrase:
        ...
        print(f"[AUTH] [OK] 从 .env 加载凭证成功 (key: {api_key[:10]}...)")
        return auth

    # .env 里没有 → 派生，打印，退出
    auth.derive_l2_credentials()  # 同步方法，不要 await
    print("[INFO] 请填入 .env 后重新运行 --live 即可。")
    sys.exit(0)
        ...
        async with self.semaphore:
            ...
            try:
                ...
                async with self.session.get(url, params=params, timeout=aiohttp.ClientTimeout(total=10)) as resp:
                    if resp.status != 200:
                        print(f"[WARN]  HTTP {resp.status} 访问 {url}")
                        return None
                ...
                print(f"[WARN]  请求失败: {type(e).__name__}: {e}")
                return None
...
async def scan_and_find_trades(api_client: ApiClient, date_range: List[datetime]) -> List[dict]:
    """扫描所有markets，找出可交易的候选（按event选best_ask最高的Yes outcome）"""
    all_slugs = []
    ...
    print(f"[SCAN] 待检查 slug 总数: {len(all_slugs)}")
    ...
    print(f"[SCAN] 收集到原始结果: {len(raw_candidates)}")
    ...
    for cand, yes_book, no_book in results:
        ...
        if not (0.90 <= complement <= 1.15):
            print(f"[WARN]  {cand['city']}/{cand['date']} 是+否={complement:.3f}, 跳过")
            continue

        # 验证通过，打印调试信息
        print(f"[DEBUG] {cand['city']}/{cand['date']} 是_卖价={yes_ask:.3f} 否_卖价={no_ask:.3f} 总和={complement:.3f} 数量={yes_ask_size:.1f}")
    ...
    print(f"[SCAN] 有效订单簿结果: {len(enriched)}")
    ...
    print(f"[SCAN] 每个事件选出最佳结果: {len(selected)}")
    ...
    print(f"[SCAN] 可交易事件 (best_ask >= {ENTRY_THRESHOLD}): {len(tradeable)}")
...
class PolymarketAuth:
    ...
    def derive_l2_credentials(self, base_url: str = "https://clob.polymarket.com", chain_id: int = 137) -> dict:
        ...
        Returns:
            凭证字典
        """
        print(f"[AUTH] 正在为 {self.wallet_address} 派生 L2 API 凭证...")
        ...
        try:
            ...
            print()
            print("=" * 64)
            print("  [OK] 凭证派生成功，请将以下内容添加到你的 .env 文件")
            print("=" * 64)
...
async def execute_buy_order(token_id: str, price: float, max_price: float, dry_run: bool = True,
    ...
    if dry_run:
        if price > max_price:
            print(f"  [DRY] 将跳过: 价格 ${price:.4f} 超过最大允许值 ${max_price:.4f} (滑点)")
            return None
        print(f"  [DRY] 将买入 1 股 @ ${price:.4f} (最高允许: ${max_price:.4f})")
        return {"dry_run": True, "token_id": token_id, "price": price, "max_price": max_price}

    # 依赖检查
    if not _HAS_PY_CLOB_CLIENT:
        print("[ERROR] py-clob-client 是实盘必需依赖")
        print("[ERROR] 请安装: pip install py-clob-client")
        return None

    # 实盘检查
    if price > max_price:
        print(f"  [ERROR] 价格 ${price:.4f} 超过最大允许值 ${max_price:.4f} (滑点)")
        return None

    if not no_token_id:
        print(f"  [ERROR] 实盘交易需要 no_token_id")
        return None

    if not auth_manager or not auth_manager.api_key:
        print(f"  [ERROR] 需要带有派生 L2 凭证的 auth_manager")
        return None
    ...
    try:
        ...
        print(f"  [LIVE] 钱包: {auth_manager.wallet_address}")
        print(f"  [LIVE] 下单中: 代币={token_id[:16]}... 价格={price:.4f} 数量={size} 成本≈${round(price*size,4)}")
        ...
        for attempt in range(retries):
            try:
                ...
                print(f"  [LIVE] 订单已提交！ID: {order_id}")
                ...
                if not dry_run and user_id:
                    try:
                        ...
                        if bill.get('ok'):
                            print(f"  [OK] 计费成功 - 余额: {bill['balance']:.4f} USDT")
                        else:
                            balance = bill.get('balance', 0)
                            payment_url = bill.get('payment_url') or billing_get_payment_link(user_id, 8)
                            # 订单已上链，不能因为计费失败而退出，只记录警告
                            print(f"\n[WARNING] 计费失败 (余额: {balance:.4f} USDT)")
                            print(f"   订单 {order_id} 已成功提交但未计费。")
                            if payment_url:
                                print(f"   请充值: {payment_url}\n")
                    except Exception as e:
                        # 计费异常不应影响订单成功
                        print(f"[WARN] 计费错误（忽略）: {e}")
                ...
                if "insufficient" in error_msg.lower() or "balance" in error_msg.lower():
                    ...
                    print(f"  [ERROR] 订单失败 (尝试 {attempt+1}): {e}")

                if attempt < retries - 1:
                    wait_time = 2 ** attempt
                    print(f"  [RETRY] 等待 {wait_time}s...")
                    await asyncio.sleep(wait_time)
                else:
                    return None

    except Exception as e:
        print(f"  [ERROR] 实盘设置失败: {e}")
        import traceback
        traceback.print_exc()
        return None
...
def print_positions(state: dict):
    ...
    for pos_key, pos in positions.items():
        ...
        print(f"\n[市场] {question}...")
        print(f"    成本: ${cost:.4f} | 股数: {shares} | 入场价: ${entry:.4f}")
        bid_str = f"${best_bid:.4f}" if best_bid is not None else "N/A"
        print(f"    买一: {bid_str} | 卖一: ${best_ask:.4f} | 中间价: ${current_price:.4f}")
        print(f"    未实现盈亏: {'+' if unrealized >=0 else ''}${unrealized:.4f}")

    print(f"\n总成本: ${total_cost:.4f}")
    print(f"总未实现: {'+' if total_unrealized >=0 else ''}${total_unrealized:.4f}")
    print(f"{'='*80}")
...
def print_trades(trades: List[dict]):
    ...
    for trade in trades[-10:][::-1]:  # Last 10 trades
        ...
        print(f"\n[时间] {ts}")
        print(f"    类型: {type_.upper()}")
        print(f"    市场: {question}...")
        print(f"    价格: ${price:.4f} | 成本: ${cost:.4f}")
...
def print_pnl(state: dict):
    ...
    print(f"总盈亏: {'+' if total_pnl >= 0 else ''}${total_pnl:.4f}")
    balance = state.get("balance", 0)
    print(f"虚拟余额: ${balance:.2f}")
    print(f"{'='*80}")

def print_full_report(state: dict, tradeable: List[dict]):
    """Print full report every 4 minutes（不清屏，保留扫描日志）"""

    now = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    print(f"\n{'#'*80}")
    print(f"# 天气狙击手 v1.0.0 - 状态报告")
    print(f"# 时间: {now}")
    print(f"{'#'*80}\n")
    ...
    print(f"{'扫描摘要':^80}")
    print(f"{'='*80}")
    print(f"可交易市场数: {len(tradeable)}")
    print(f"监控城市数: {len(CITY_SLUGS)}")
    print(f"日期范围: 今天 + 明天")
    print(f"交易金额: ${TRADE_AMOUNT_USD} (最高允许: ${MAX_COST_PER_TRADE})")
    print(f"回退阈值: {ENTRY_THRESHOLD} (由扫描强制执行)")
    print(f"{'='*80}\n")
...
class SniperBot:
    ...
    async def run(self):
        """主循环"""
        print("\n" + "="*60)
        print("[START] 高温天气狙击手 v1.0.0")
        print("="*60)
        print(f"模式: {'模拟运行' if self.dry_run else '实盘运行'}")
        print(f"监控城市数: {len(CITY_SLUGS)}")
        print(f"日期范围: 2 天 (今天 + 明天)")
        end_h = int(MONITOR_END_HOUR)
        end_m = int((MONITOR_END_HOUR - end_h) * 60)
        print(f"监控窗口: 本地 {MONITOR_START_HOUR}:00-{end_h}:{end_m:02d}")
        print(f"扫描间隔: {CHECK_INTERVAL//60} 分钟")
        print(f"报告间隔: {REPORT_INTERVAL//60} 分钟")
        print("="*60 + "\n")
        ...
        if (self.once or not self.dry_run) and self.auth_manager:
            if not self.auth_manager.api_key:
                try:
                    ...
                    # 派生成功后退出，让用户填入 .env
                    print("[INFO] 请将上面的凭证填入 .env 文件，然后重新运行。")
                    return
                except Exception as e:
                    print(f"[ERROR] 认证初始化失败: {e}")
                    print("[ERROR] 请检查你的 PRIVATE_KEY 和 PROXY_WALLET。")
                    return
            else:
                print("[AUTH] 凭证已从 .env 加载")

        async with ApiClient() as api_client:
            while self.running:
                now = datetime.now(timezone.utc)

                # --once 模式：进入循环立即执行一次扫描并退出
                if self.once:
    ...
    async def scan_cycle(self, api_client: ApiClient):
        ...
        print(f"\n[{datetime.now(timezone.utc).strftime('%H:%M:%S')}] 开始扫描...")

        # 动态重算 date_range（只扫描今明两天，减少API调用）
        self.date_range = [
        ...
        print(f"[SCAN] 监控窗口内可交易: {len(window_candidates)}")
        ...
        for cand in window_candidates:
            ...
            if not self.dry_run and SKILLPAY_USER_ID:
                try:
                    balance = billing_get_balance(SKILLPAY_USER_ID)
                    if balance < MIN_CHARGE_AMOUNT:
                        print(f"\n[ERROR] 余额不足（{balance:.4f} < {MIN_CHARGE_AMOUNT} USDT），停止本轮剩余下单\n")
                        break  # 退出循环，不再继续
                except Exception as e:
            ...
            print(f"[BUY] 触发: {cand['question'][:60]}...")
            print(f"    城市: {cand['city']} 日期: {cand['date']}")
            print(f"    卖一: ${cand['best_ask']:.4f} 最高允许: ${max_price:.4f} 成本: ${cand['cost_usd']:.4f}")
            ...
            if order_result:
                ...
                new_positions += 1
                print(f"  [OK] 买入成功 ({'模拟' if self.dry_run else '实盘'})")
        ...
        print(f"[SCAN] 新建仓位数: {new_positions}")
        ...
        for city in CITY_SLUGS:
            ...
            if not self.dry_run and SKILLPAY_USER_ID:
                try:
                    balance = billing_get_balance(SKILLPAY_USER_ID)
                    if balance < MIN_CHARGE_AMOUNT:
                        print(f"\n[ERROR] 余额不足（{balance:.4f} < {MIN_CHARGE_AMOUNT} USDT），停止回退剩余下单\n")
                        break  # 退出fallback循环
                except Exception as e:
                    print(f"[WARN] 余额检查失败（{e}），继续...")
            ...
            if not city_candidates:
                print(f"[FALLBACK] {city} {today_str} 无候选")
                continue
            ...
            print(f"\n[FALLBACK] 已触发 {city} {today_str}")
            print(f"    候选: {best['question'][:60]}... (中间价=${best.get('clob_mid_price', 0):.4f}, Gamma价={best.get('gamma_price', 0):.4f})")
    ...
    async def print_report_cycle(self, api_client: ApiClient):
        ...
        if positions:
            print(f"[REPORT] 更新 {len(positions)} 个仓位价格...")
            for pos_key, pos in positions.items():
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="天气高温狙击手 v1.0.0")
    parser.add_argument("--dry-run", action="store_true",
                        help="模拟模式 (不实际下单)")
    parser.add_argument("--live", action="store_true",
                        help="实盘交易 (需要配置 PRIVATE_KEY 和 PROXY_WALLET)")
    parser.add_argument("--once", action="store_true",
                        help="运行一次扫描周期后退出 (调试用)")
