...
功能：检测 Python 版本、Skill 版本更新、密钥、角色配置状态，输出检测结果
...
# 远端版本查询 API（通过 slug 查询最新发布版本）
REMOTE_VERSION_URL = "https://api.codebuddy.ai/skills/{slug}/latest-version"
...
def get_remote_version(slug: str) -> str | None:
    """从远端 API 查询指定 slug 的最新版本号，返回版本字符串或 None"""
    url = REMOTE_VERSION_URL.format(slug=slug)
    try:
        req = urllib.request.Request(url, method="GET")
        req.add_header("Accept", "application/json")
        with urllib.request.urlopen(req, timeout=VERSION_CHECK_TIMEOUT) as resp:
            data = json.loads(resp.read().decode("utf-8"))
            return data.get("version") or data.get("latest_version")
