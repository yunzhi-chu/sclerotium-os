...
```bash
python "${CLAUDE_SKILL_DIR}/scripts/compdf_conversion_cli.py" excel input.pdf output.xlsx --excel-all-content --excel-worksheet-option for-page
```

### PDF to TXT, with table formatting enabled
```bash
python "${CLAUDE_SKILL_DIR}/scripts/compdf_conversion_cli.py" txt input.pdf output.txt
```

### PDF to HTML, multi-page with bookmarks mode
```bash
python "${CLAUDE_SKILL_DIR}/scripts/compdf_conversion_cli.py" html input.pdf output_dir --html-option multiple-page-with-bookmark
```

### PDF to Image, PNG format, grayscale, 2x scaling
```bash
python "${CLAUDE_SKILL_DIR}/scripts/compdf_conversion_cli.py" image input.pdf output.png --image-type png --image-color-mode gray --image-scaling 2.0
```
