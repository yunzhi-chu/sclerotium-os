...
XGOorder = {
    ...
    "WHEEL_CTRL_FLAG": [0xB0, 0x0],
    "WHEEL_CTRL": [0xB1, 0x80, 0x80, 0x80, 0x80],
}
...
class XGO_DOG():
    ...
    def enable_wheel_control(self, mode):
        if mode != 0 and mode != 1:
            print("ERROR!Illegal Value!")
            return
        XGOorder["WHEEL_CTRL_FLAG"][1] = mode
        self.__send("WHEEL_CTRL_FLAG")

    def wheel_control(self, data):
        for i in range(4):
            XGOorder["WHEEL_CTRL"][i + 1] = data[i]
            self.__send("WHEEL_CTRL", len=4)
