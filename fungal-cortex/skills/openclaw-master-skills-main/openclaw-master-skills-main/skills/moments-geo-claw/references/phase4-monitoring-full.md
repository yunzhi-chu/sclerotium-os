...
**客户名称**: 报告中所有品牌名称、竞品名称必须使用会话中客户提供的真实名称。`{品牌名}` 是模板变量，输出前必须全部替换为实际品牌名（如"元気森林"、"百威中国"）。含有未替换占位符的报告不得交付客户。

**AI测试数据**: 监控报告中的AI平台测试数据必须来自Playwright MCP实际执行结果。如无法执行真实测试，所有平台数据必须标注`(预估)`并说明原因。
...
(v1.0 初稿)              (v2.0 定稿)              (内容输出)                (追踪变化)
    │                          │                           │                          │
01_diagnosis/             02_positioning/            03_contents/              04_monitoring/
```
