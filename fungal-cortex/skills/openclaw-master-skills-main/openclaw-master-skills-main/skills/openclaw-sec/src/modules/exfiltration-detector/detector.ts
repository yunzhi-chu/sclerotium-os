...
import { exfiltrationPatterns } from '../../patterns/runtime-validation/exfiltration-patterns';
...
export class ExfiltrationDetector {
  ...
  constructor(config: ModuleConfig) {
    ...
    this.patterns = exfiltrationPatterns.filter(pattern => pattern.enabled);
  }
  ...
  async scan(text: string): Promise<Finding[]> {
    ...
    for (const pattern of this.patterns) {
      ...
      if (match) {
        findings.push({
          module: 'exfiltration_detector',
          pattern: pattern,
          matchedText: match[0].length > 100 ? match[0].substring(0, 100) + '...' : match[0],
