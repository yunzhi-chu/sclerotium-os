...
import { codeExecutionPatterns } from '../../patterns/runtime-validation/code-execution-patterns';

export class CodeExecutionDetector {
  ...
  constructor(config: ModuleConfig) {
    ...
    this.patterns = codeExecutionPatterns.filter(pattern => pattern.enabled);
  }
  ...
  async scan(text: string): Promise<Finding[]> {
    ...
    for (const pattern of this.patterns) {
      ...
      if (match) {
        findings.push({
          module: 'code_execution_detector',
          pattern: pattern,
          matchedText: match[0],
