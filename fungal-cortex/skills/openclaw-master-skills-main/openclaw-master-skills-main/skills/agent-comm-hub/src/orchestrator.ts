...
// better-sqlite3 prepare<T>() 的 T 是 BindParameters 非 Result
...
function updatePipelineStatus(pipelineId: string, status: string, operatorId: string): Pipeline {
  const pipeline = getOne<Pipeline>(`SELECT * FROM pipelines WHERE id=?`, pipelineId);
  ...
  return getOne<Pipeline>(`SELECT * FROM pipelines WHERE id=?`, pipelineId)!;
}
...
function cascadeDependencySatisfaction(completedTaskId: string): void {
  ...
  for (const d of downstreams) {
    const task = getOne<Task>(`SELECT * FROM tasks WHERE id=?`, d.downstream_id);
...
export function addDependency(
  ...
  operatorId?: string
): { dependency: ReturnType<typeof taskRepo.addDependency>; downstream_updated: boolean } {
  // 验证任务存在
  const upstream = getOne<Task>(`SELECT * FROM tasks WHERE id=?`, upstreamId);
  if (!upstream) throw new Error(`Upstream task not found: ${upstreamId}`);
  const downstream = getOne<Task>(`SELECT * FROM tasks WHERE id=?`, downstreamId);
...
export function getDependencies(taskId: string): {
  ...
  const mapDep = (dep: typeof upstreams[number]) => {
    const task = getOne<Task>(`SELECT id, status FROM tasks WHERE id=?`, dep.downstream_id);
    return {
  ...
  const mapUp = (dep: typeof upstreams[number]) => {
    const task = getOne<Task>(`SELECT id, status FROM tasks WHERE id=?`, dep.upstream_id);
    return {
...
export function createParallelGroup(
  ...
  for (const taskId of taskIds) {
    const task = getOne<Task>(`SELECT id FROM tasks WHERE id=?`, taskId);
    if (!task) throw new Error(`Task not found: ${taskId}`);
...
export function addQualityGate(
  ...
  afterOrder: number,
  operatorId: string
): { gate: ReturnType<typeof taskRepo.addQualityGate>; pipeline_id: string } {
  // 验证 Pipeline 存在
  const pipeline = getOne<Pipeline>(`SELECT * FROM pipelines WHERE id=?`, pipelineId);
  if (!pipeline) throw new Error(`Pipeline not found: ${pipelineId}`);
  ...
  if (status === "failed") {
    ...
    for (const t of laterTasks) {
      if (t.status !== "waiting") {
        ...
        const task = getOne<Task>(`SELECT assigned_to FROM tasks WHERE id=?`, t.task_id);
        if (task?.assigned_to) {
          pushToAgent(task.assigned_to, {
